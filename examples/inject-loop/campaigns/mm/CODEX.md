...
1. `ralph/examples/inject-loop/campaigns/mm/campaign.json` - campaign config and current status
2. `ralph/examples/inject-loop/campaigns/mm/progress.txt` - append-only campaign log, if it exists
...
Read `ralph/examples/inject-loop/campaigns/mm/campaign.json` and identify:
...
- set `status` to `paused` or `completed` in `ralph/examples/inject-loop/campaigns/mm/campaign.json`
- write or update `PAUSED.md` in the loop directory
- append the reason to `ralph/examples/inject-loop/campaigns/mm/progress.txt`
...
python3 experiments/lib/live_mix.py \
  --campaign ralph/examples/inject-loop/campaigns/mm/campaign.json \
  --refresh-supported
```
...
For normal loop operation, prefer `python3 experiments/lib/live_mix.py --campaign ralph/examples/inject-loop/campaigns/mm/campaign.json --refresh-supported` because it refreshes the supported catalog and applies the campaign's hard exclusions before writing `_supported_candidates.json`.
...
Update `ralph/examples/inject-loop/campaigns/mm/campaign.json` to point at the newly active round and append a short iteration log to `ralph/examples/inject-loop/campaigns/mm/progress.txt`.

## Progress Log Format

Append to `ralph/examples/inject-loop/campaigns/mm/progress.txt`:
