...
1. `ralph/examples/inject-loop/campaigns/otel-demo/campaign.json` - campaign config and current status
2. `ralph/examples/inject-loop/campaigns/otel-demo/progress.txt` - append-only campaign log, if it exists
...
7. On the current cluster, all `HTTP*` chaos types are hard-disabled. Never plan or submit them even if Aegis lists them in the raw supported catalog.
8. If the system shows persistent low signal or environmental failure, pause instead of grinding more rounds.

## Per-Iteration Workflow

### 1. Load campaign state

Read `ralph/examples/inject-loop/campaigns/otel-demo/campaign.json` and identify:

- target `system`
- target `backend_system` for `aegisctl` calls if present, otherwise reuse `system`
- target `project`
- target `namespace`
...
- set `status` to `paused` or `completed` in `ralph/examples/inject-loop/campaigns/otel-demo/campaign.json`
- write or update `PAUSED.md` in the loop directory
- append the reason to `ralph/examples/inject-loop/campaigns/otel-demo/progress.txt`
...
1. Refresh the live system snapshot first:

```bash
python3 experiments/lib/live_mix.py \
  --campaign ralph/examples/inject-loop/campaigns/otel-demo/campaign.json \
  --refresh-supported
```

Read `live_mix.json` before allocating the next round. Use it to answer:

- which `chaos_type` values the system supports
- which `chaos_type` values are already overrepresented in current injections
- which traces are still Running / Pending
- which `chaos_type` values are missing from injections or trace outcomes
- which `chaos_type` values are filtered out by campaign policy, especially the cluster-wide `HTTP*` ban

2. Enumerate supported candidates with:

```bash
aegisctl inject candidates ls --system <backend_system_or_system> --namespace <ns> -o json \
  > <loop_dir>/_supported_candidates.json
```

For normal loop operation, prefer `python3 experiments/lib/live_mix.py --campaign ralph/examples/inject-loop/campaigns/otel-demo/campaign.json --refresh-supported` because it refreshes the supported catalog and applies the campaign's hard exclusions before writing `_supported_candidates.json`.

3. Compute the supported `chaos_type` set.
4. Count picks across the last 3 rounds.
5. Allocate `batch_size` slots by `chaos_type` before selecting concrete candidates.
6. Enforce the pod-family cap and missing-type floor.
7. Reduce budget for `chaos_type` values that already have a large Running backlog in `live_mix.json`.
8. Set one round-level `defaults.duration` from the campaign config and keep it fixed.
9. Only after the type budget is fixed, choose per-type candidates and vary params / interval / pre_duration to avoid dedup.
...
Update `ralph/examples/inject-loop/campaigns/otel-demo/campaign.json` to point at the newly active round and append a short iteration log to `ralph/examples/inject-loop/campaigns/otel-demo/progress.txt`.

## Progress Log Format

Append to `ralph/examples/inject-loop/campaigns/otel-demo/progress.txt`:
