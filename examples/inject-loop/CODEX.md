...
1. `ralph/examples/inject-loop/campaign.json` - campaign config and current status
2. `ralph/progress.txt` - append-only campaign log, if it exists
3. `.codex/skills/inject-loop/SKILL.md` - if present in the target repo, treat it as authoritative workflow guidance

If you keep this example under the default Ralph checkout, the campaign file path is `ralph/examples/inject-loop/campaign.json`.
...
Read `ralph/examples/inject-loop/campaign.json` and identify:
...
- set `status` to `paused` or `completed` in `ralph/examples/inject-loop/campaign.json`
- write or update `PAUSED.md` in the loop directory
- append the reason to `ralph/progress.txt`
...
For normal loop operation, prefer `live_mix.py` because it refreshes the supported catalog and applies the campaign's hard exclusions before writing `_supported_candidates.json`.
...
7. Reduce budget for any `chaos_type` already overrepresented in `live_mix.json`.
...
Update `ralph/examples/inject-loop/campaign.json` to point at the newly active round and append a short iteration log to `ralph/progress.txt`.
