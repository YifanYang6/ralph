...
1. `{{CAMPAIGN_FILE}}` - campaign config and current status
2. `{{PROGRESS_FILE}}` - append-only campaign log, if it exists
...
Read `{{CAMPAIGN_FILE}}` and identify:
...
- set `status` to `paused` or `completed` in `{{CAMPAIGN_FILE}}`
- write or update `PAUSED.md` in the loop directory
- append the reason to `{{PROGRESS_FILE}}`
...
For normal loop operation, prefer `python3 experiments/lib/live_mix.py --campaign {{CAMPAIGN_FILE}} --refresh-supported` because it refreshes the supported catalog and applies the campaign's hard exclusions before writing `_supported_candidates.json`.
...
Update `{{CAMPAIGN_FILE}}` to point at the newly active round and append a short iteration log to `{{PROGRESS_FILE}}`.
